  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'backdrop-blur-lg bg-white/95 dark:bg-black/90 border-b border-gray-200/50 dark:border-gray-800/50 shadow-lg'
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-gray-900 dark:text-white hover:text-[#a57614] dark:hover:text-[#d4a574] transition-colors ${
                  isActive(link.path)
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-md text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900/50 transition-all"
          ...
          <div className="px-4 py-4 space-y-4">
            {navLinks.map((link) => (
              <Link
                ...
                className={`block text-gray-900 dark:text-white hover:text-[#a57614] dark:hover:text-[#d4a574] transition-colors ${
                  isActive(link.path)
                    ? 'text-[#a57614] dark:text-[#d4a574] font-semibold'
                    : ''
                }`}
